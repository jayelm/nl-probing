...
def load_snli_labels(args: DictConfig, tokenizer: PreTrainedTokenizer) -> DatasetDict:
    """Load an SNLI-type dataset with a "premise" and "hypothesis" field."""
    datasets = load_dataset(args.data.dataset_name, args.data.dataset_config_name)
...
def load(args: DictConfig, tokenizer: PreTrainedTokenizer, keep_labels = False) -> DatasetDict:
    if "snli" in args.data.dataset_name:
        if keep_labels:
            return load_snli_labels(args, tokenizer)
        else:
